...
def validate(ev):
    """Normalise an event from the network; None if it is malformed."""
    if not isinstance(ev, dict):
        return None
    a, s, k = str(ev.get("a", "")), ev.get("s"), ev.get("k")
    if not UID_RE.match(a) or not isinstance(s, int) or s < 1 or k not in KINDS:
        return None
...
class Store:
    def __init__(self, room_dir, me):
        self.me = me
        self.log_path = room_dir / "events.jsonl"
        self.events = {}
    ...
    def missing_for(self, vec):
        ...
        for ev in self.events.values():
            if ev["s"] > int(vec.get(ev["a"], 0) or 0):
                out.append(ev)
        out.sort(key=lambda e: (e["a"], e["s"]))
        return out

    def add(self, ev):
        """Store an event from the network. Returns the normalised event if it was new."""
        ev = validate(ev)
    ...
    def create(self, event_kind, **payload):
        ...
        ev = {"id": f"{self.me}:{seq}", "a": self.me, "s": seq, "k": event_kind,
              "ts": now_ms(), **payload}
        return self.add(ev)
